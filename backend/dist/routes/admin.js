...
function adminRoutes(prisma, redis) {
    const router = (0, express_1.Router)();
    /**
     * POST /api/admin/flash-sale
     * Create a new flash sale
     */
    router.post("/flash-sale", (0, validation_1.validateRequest)({ body: schema_1.flashSaleSchema }), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
        const { productName, productDescription, originalPrice, flashSalePrice, currency = "USD", totalStock, startTime, endTime, } = req.body;
     ...
     */
    router.put("/flash-sale/:id", (0, validation_1.validateRequest)({
        params: schema_1.uuidParamSchema,
        body: schema_1.flashSaleSchema,
    }), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
        const { productName, productDescription, originalPrice, flashSalePrice, currency, totalStock, startTime, endTime, } = req.body;
        const existingSale = await prisma.flashSale.findUnique({
            where: { id },
        });
        if (!existingSale) {
            throw new errors_1.NotFoundError("Flash sale");
        }
     ...
     */
    router.post("/flash-sale/:id/activate", (0, validation_1.validateRequest)({
        params: schema_1.uuidParamSchema,
        body: schema_1.activateSchema,
    }), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
        ...
        res.json({
            success: true,
            message: `Flash sale ${isActive ? "activated" : "deactivated"} successfully`,
            data: flashSale,
        });
    }));
    /**
     ...
     */
    router.get("/flash-sales", (0, asyncHandler_1.asyncHandler)(async (req, res) => {
     ...
     */
    router.get("/flash-sale/:id/analytics", (0, validation_1.validateRequest)({ params: schema_1.uuidParamSchema }), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
        ...
        const analytics = {
            ...
            purchases: {
                ...
                successful: flashSale.purchases.filter((p) => p.status === "COMPLETED").length,
                failed: flashSale.purchases.filter((p) => p.status === "FAILED")
                    .length,
                firstPurchase: flashSale.purchases[0]?.purchasedAt,
                lastPurchase: flashSale.purchases[flashSale.purchases.length - 1]?.purchasedAt,
            },
     ...
     * Delete a flash sale
     */
    router.delete("/flash-sale/:id", (0, validation_1.validateRequest)({ params: schema_1.uuidParamSchema }), (0, asyncHandler_1.asyncHandler)(async (req, res) => {
        const { id } = req.params;
