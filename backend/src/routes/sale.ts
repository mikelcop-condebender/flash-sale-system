...
import { FlashSale, SaleStatusParams } from "../types";
import { saleStatusParamsSchema } from "../schema";
...
export function saleRoutes(
  ...
  router.get(
    "/status/:flashSaleId",
    validateRequest({ params: saleStatusParamsSchema }),
    asyncHandler(async (req: Request<SaleStatusParams>, res: Response) => {
...
function determineSaleStatus(flashSale: any, now: Date): string {
